...
import com.faichuis.faichuismall.common.common.LocalCache;
...
import com.faichuis.faichuismall.common.uitls.RedisOpsUtil;
import com.faichuis.faichuismall.mapper.OmsOrderItemMapper;
import com.faichuis.faichuismall.mapper.OmsOrderMapper;
...
import com.faichuis.order.PmsProductFeignApi;
import com.faichuis.order.SecKillOrderService;
import com.faichuis.order.TradeService;
import com.faichuis.order.UmsMemberFeignApi;
...
//                    orderMessageSender.sendCreateOrderMsg(orderMessage);
            if(sendStatus){
                /*
                 * 打上排队的标记
                 */
...
//            cache.remove(RedisKeyPrefixConst.MIAOSHA_STOCK_CACHE_PREFIX + productId);
    ...
    public Long asyncCreateOrder(OmsOrder order,OmsOrderItem orderItem,Long flashPromotionRelationId) {
        //减库存
        Integer result = 1;
//                miaoShaStockDao.descStock(flashPromotionRelationId, 1);
        if (result <= 0) {
...
//        orderItemMapper.insertSelective(orderItem);
        ...
        try {
            boolean sendStatus = true;
//                    orderMessageSender.sendTimeOutOrderMessage(order.getId() + ":" + flashPromotionRelationId + ":" + orderItem.getProductId());
            if (!sendStatus) {
...
//        cache.getCache(RedisKeyPrefixConst.MIAOSHA_STOCK_CACHE_PREFIX + productId);
